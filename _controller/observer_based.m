...
for k = 1:NumStep-1
    ...
    %prepareation for QP
            ...
            gamma = 10;
            p = 100;
            partial_h_0_partial_x = [1,-1,0,0,0,0];
            partial_h_1_e_partial_x = [-1,1,1,-1,0,0];
            partial_h_2_e_partial_x = [-1,1,0,0,1,-1];
     % h_i = s_i - v_i;
    ...
    if k > ActuationTime/Tstep
        u_0 =  K*X(:,k);
       % QP for STC controller   
    [sigma_1,sigma_2]=deal(0);
    % Define the objective function
        objfun = @(x) (x(1)-u_0)^2 + p*(x(2)^2 + x(3)^2);
    ...
    % Linear inequality constraints
        ...
        A_QP = [1, 0, 0;
            ...
            h_0 = S(k,m+1,1) - tao*S(k,m+1,2);
            h_1 = S(k,m+2,1) - tao*S(k,m+2,2);
            h_2 = S(k,m+3,1) - tao*S(k,m+3,2);
            h_bar_1 = h_1 -  h_0;
            h_bar_2 = h_2 -  h_0;
            
        b = [ partial_h_0_partial_x*A*X(:,k)+gamma*h_0; 
              partial_h_1_e_partial_x*A*X(:,k)+gamma*h_bar_1;
              partial_h_2_e_partial_x*A*X(:,k)+gamma*h_bar_2];
        ...
        % No bounds
        lb = [-inf; 0; 0];  % Lower bounds: No bound on u, sigma1 > 0, sigma2 > 0, No bound on sigma3
        ub = [inf; inf; inf]; % Upper bounds: All variables can go to positive infinity

        % Use fmincon to solve the problem
        options = optimoptions('fmincon','Display','iter');
        [x,fval] = fmincon(objfun,x0,A_QP,b,Aeq,Beq,lb,ub,[],options);
